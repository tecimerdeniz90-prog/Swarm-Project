    ...
    def run_simulation(self):
        ...
        while step < self.max_steps and not all_reached:
            ...
            for agent in self.agents:
                ...
                if dist > 1.0:  # Hedef toleransı (Kinodynamic planner 1.0m ile uyumlu)
                    all_reached = False
                    break
        ...
        for i in range(num_agents):
            if i not in visited:
                ...
                if has_conflict:
                    ...
                    if len(cluster) > 1:
                        # ECBS'in yavaşlamaması için küme boyutunu maksimum 4 İHA ile sınırla
                        if len(cluster) > 4:
                            cluster = sorted(cluster)[:4]
