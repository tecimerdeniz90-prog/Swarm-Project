...
def visualize_swarm(map_path, starts, goals, history, links_history, leader_history, save_path="simulation_swarm.gif"):
    ...
    points_ds = points[::35] if len(points) > 0 else np.array([])
    
    fig = plt.figure(figsize=(15, 12))
    ax = fig.add_subplot(111, projection='3d')
    ax.set_title("Dağıtık Çoklu İHA Sürü Yol Planlama\n(60 İHA P2P İletişim ve Dinamik Liderlik)", fontsize=14, fontweight='bold', pad=20)
    ...
    if len(points_ds) > 0:
        ax.scatter(points_ds[:, 0], points_ds[:, 1], points_ds[:, 2], 
                   c='dimgray', s=12.0, alpha=0.5, label='Engeller (Nokta Bulutu)')
    ...
    for i in range(num_agents):
        # Başlangıç
        ax.scatter(starts[i][0], starts[i][1], starts[i][2], c='green', marker='o', s=40 if num_agents > 10 else 80, edgecolors='black', zorder=5)
        if num_agents <= 5:
            ax.text(starts[i][0], starts[i][1], starts[i][2] + 0.3, f"Start {i}", color='green', weight='bold', fontsize=9)
        # Hedef
        ax.scatter(goals[i][0], goals[i][1], goals[i][2], c='red', marker='*', s=60 if num_agents > 10 else 120, edgecolors='black', zorder=5)
        if num_agents <= 5:
